...
echo "Cloning"
git clone $ORG/$1 -b $2 $KERNEL_DIR/$1 --depth 1 || {
	echo "Your device is not officially supported or wrong branch"
	exit 1
	rm -rf $PROJECT_DIR/kernelsource
}

# Find defconfig
echo "Checking if defconfig exist ($1)"

if [ -f $KERNEL_DIR/$1/arch/arm64/configs/$1-perf_defconfig ]; then
	echo "Starting build"
elif [ -f $KERNEL_DIR/$1/arch/arm64/configs/vendor/$1-perf_defconfig ]; then
	echo "Starting build"
else
...
if [ -f $KERNEL_DIR/$1/out/arch/arm64/boot/Image.gz-dtb ]; then
	echo "Build Complete"
else
...
# Build dtbo
if [ -f $KERNEL_DIR/$1/out/arch/arm64/boot/dts/xiaomi/*.dtbo ]; then
	echo "Building DTBO"
